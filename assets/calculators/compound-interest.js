function calcCompound(){
 ...
 const C=window.CMFChart&&CMFChart.colors;if(C&&Number.isFinite(balance))CMFChart.area(document.getElementById('compound-chart'),labels,[{label:'Your contributions',color:C.brand,values:deposits},{label:'Interest earned',color:C.gold,values:growth}]);
 setMessage('compound-note',rate===0?'With a 0% rate, the result is simply the starting amount plus contributions.':`About ${Number.isFinite(balance)&&balance>0?Math.round(earned/balance*100):0}% of your final balance comes from compound growth. This assumes a constant rate; real returns vary and aren't guaranteed.`);
}
